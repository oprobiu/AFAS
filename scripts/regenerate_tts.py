#!/usr/bin/env python3
"""
Regenerate TTS audio using edge-tts. Fully config-driven.

Reads tts config from deck.json:
  - tts.engine: "edge-tts" (only supported engine)
  - tts.voices: list of voice names
...
def make_filename(prefix, text, voice):
    h = hashlib.sha1(f"{prefix}:{voice}:{text}".encode("utf-8")).hexdigest()
    return f"edgetts-{prefix}-{h[:8]}-{h[8:16]}-{h[16:24]}-{h[24:32]}-{h[32:40]}.mp3"


def make_dialogue_filename(prefix, text):
    h = hashlib.sha1(f"dialogue:{prefix}:{text}".encode("utf-8")).hexdigest()
    return f"edgetts-{prefix}-{h[:8]}-{h[8:16]}-{h[16:24]}-{h[24:32]}-{h[32:40]}.mp3"


async def generate_audio(text, voice, path, retries=3):
...
async def generate_dialogue(text, dialogue_cfg, voices, path):
    ...
    try:
        await generate_audio(part_a, female_voice, tmp_a)
        await generate_audio(part_b, male_voice, tmp_b)
...
async def process_target(target, notes, voices, media_dir, args):
    ...
    for i, row in enumerate(notes):
        ...
        if is_dlg:
            filename = make_dialogue_filename(prefix, text)
        else:
            filename = make_filename(prefix, text, voice)
        ...
        if os.path.exists(path):
            ...
            label = "DIALOGUE" if is_dlg else voice.split("-")[-1]
            display = text[:55] + ("..." if len(text) > 55 else "")
            print(f"  [{i}] {label}: {display}", flush=True)
            if not args.dry_run:
                if is_dlg:
                    await generate_dialogue(text, dialogue_cfg, voices, path)
                else:
...
async def run(args):
    ...
    for target in targets:
        total_gen += await process_target(target, process_notes, voices, media_dir, args)
